...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function isLearned(card){
    return card.interval >= 21; // ۳ هفته یا بیشتر
  }
  ...
  function renderDecks(){
    ...
    decks.forEach(deck => {
      ...
      const learnedRatio = s.total > 0 ? Math.round((s.learned / s.total) * 100) : 0;
      ...
      el.innerHTML = `
        <div class="fc-deck-top">
          ...
          <button class="fc-deck-edit" title="تنظیمات دسته" data-edit="${deck.id}">
        ...
        <div class="fc-deck-meta">
          <span>✅ ${toFa(s.learned)} یاد گرفته</span>
        </div>
        <div class="fc-deck-bar">
          <div class="fc-deck-bar-fill" style="width:${learnedRatio}%"></div>
        </div>
      `;
