...
function bindLogin(){
  ...
  form.addEventListener('submit', e => {
    ...
    let role = 'student';
var _stored = localStorage.getItem('piramid_owner_hash') || _oh(OWNER_PASSWORD);
if(_oh(pass) === _stored) role = 'owner';
    else if(pass && pass.length < 3){ window.showToast && window.showToast('رمز کوتاه است','error'); return; }
    ...
    setRole(role);
try{
  localStorage.removeItem('piramid_locked_reason');
  localStorage.setItem('piramid_last_activity', String(Date.now()));
}catch(e){}
if(window.PiramidSessionLock) window.PiramidSessionLock.reset();
    overlay.classList.remove('open');
    overlay.dataset.locked = '';
    updateHeaderAccount();

    if(role === 'owner'){
      window.showToast && window.showToast('خوش آمدی مالک پیرامید!','success',4000);
    } else {
...
function bindAccountPill(){
  ...
  pill.addEventListener('click', e => {
    ...
    if(!u){
      e.preventDefault();
      sessionStorage.setItem('piramid_return', 'account.html');
      const overlay = $('#loginOverlay');
      if(overlay){
        overlay.dataset.locked = '1';
      ...
      window.showToast && window.showToast('اول وارد شو','info', 2500);
    }
  });
}
/* ===================== 4) اعلانات ===================== */
