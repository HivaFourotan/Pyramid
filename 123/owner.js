...
function renderMessages(){
  ...
  box.innerHTML =
    '<div class="owner-card glass" style="margin-bottom:20px;">' +
      ...
      (msgs.length > 0 ?
        '<div class="owner-search">' +
          '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4-4"/></svg>' +
          '<input type="text" id="msgSearch" placeholder="جستجو در پیام‌ها (نام، ایمیل، متن)...">' +
        '</div>' : ''
      ) +
      '<div id="msgsList"></div>' +
    '</div>' +
    '<div class="owner-card glass">' +
      '<div class="owner-card-head">' +
        ...
        (bugs.length > 0 ?
          '<div class="owner-search" style="max-width:240px;margin:0;">' +
            '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4-4"/></svg>' +
            '<input type="text" id="bugSearch" placeholder="جستجو...">' +
          '</div>' : ''
        ) +
      '</div>' +
      '<div id="bugsList"></div>' +
  ...
  function paintMsgs(filter){
    var f = (filter || '').trim().toLowerCase();
    var list = msgs;
    if(f){
      list = msgs.filter(function(m){
        return (String(m.name||'')).toLowerCase().indexOf(f) !== -1 ||
               (String(m.email||'')).toLowerCase().indexOf(f) !== -1 ||
               (String(m.msg||'')).toLowerCase().indexOf(f) !== -1;
      });
    }

    var container = document.getElementById('msgsList');
    if(!container) return;

    if(list.length === 0){
      container.innerHTML = '<p class="owner-empty">' + (f ? 'چیزی پیدا نشد' : 'هنوز پیامی نیومده') + '</p>';
      return;
    }

    container.innerHTML = '<div class="owner-msg-list">' + list.map(function(m){
      var realIdx = msgs.indexOf(m);
      return '<div class="owner-msg">' +
        ...
        '<div class="owner-msg-foot">' +
          ...
          '<button class="owner-mini-btn danger" data-del="' + realIdx + '">حذف</button>' +
        '</div>' +
      '</div>';
    }).join('') + '</div>';
  ...
  function paintBugs(filter){
    var f = (filter || '').trim().toLowerCase();
    var list = bugs;
    if(f){
      list = bugs.filter(function(b){
        return (String(b.page||'')).toLowerCase().indexOf(f) !== -1 ||
               (String(b.desc||'')).toLowerCase().indexOf(f) !== -1;
      });
    }

    var container = document.getElementById('bugsList');
    if(!container) return;

    if(list.length === 0){
      container.innerHTML = '<p class="owner-empty">' + (f ? 'چیزی پیدا نشد' : 'گزارشی نیومده') + '</p>';
      return;
    }

    container.innerHTML = '<div class="owner-msg-list">' + list.map(function(b){
      var realIdx = bugs.indexOf(b);
      return '<div class="owner-msg">' +
        '<div class="owner-msg-head">' +
        ...
        '<div class="owner-msg-foot">' +
          (!b.resolved ? '<button class="owner-mini-btn" data-resolve="' + realIdx + '">حل شد</button>' : '') +
          '<button class="owner-mini-btn danger" data-bdel="' + realIdx + '">حذف</button>' +
        '</div>' +
      '</div>';
    }).join('') + '</div>';

    container.querySelectorAll('[data-resolve]').forEach(function(b){
      b.addEventListener('click', function(){
  ...
  if(msgSearch) msgSearch.addEventListener('input', function(e){ paintMsgs(e.target.value); });

  var bugSearch = document.getElementById('bugSearch');
  if(bugSearch) bugSearch.addEventListener('input', function(e){ paintBugs(e.target.value); });

  var clearBtn = document.getElementById('clearMsgs');
...
var RENDERERS = {
  ...
  stories: function(){
    var box = document.getElementById('panelStories');
    if(!box) return;
    if(window.PiramidStoriesAdmin && window.PiramidStoriesAdmin.render){
      window.PiramidStoriesAdmin.render(box);
    }
  }
};
